...
function renderResult(game, phase, session) {
  ...
  app.innerHTML = `<section class="activity-card result-card"><div class="result-mark" aria-hidden="true">✓</div><h1>${plan && count === 5 ? 'Treino concluído!' : phase && !next ? 'Percurso concluído!' : 'Muito bem!'}</h1><p>${plan && count === 5 ? 'Parabéns! Você concluiu o treino de hoje.' : phase && !next ? 'Parabéns! Você concluiu as 3 fases deste jogo.' : 'Você concluiu esta atividade no seu ritmo.'}</p><p class="result-stars" aria-label="${session.stars} estrelas recebidas"><span aria-hidden="true">${'★'.repeat(session.stars)}</span></p><p class="result-score"><strong>Pontuação:</strong> ${correct} acertos</p><p>${total} tentativas realizadas.</p>${plan && count < 5 ? `<p>Atividade ${count} de 5 concluída.</p>` : ''}<div class="actions result-actions"><button id="result-next">${primary}</button><button class="secondary" id="repeat-activity">Jogar novamente</button><a class="quiet-link" href="#/atividades">Sair do jogo</a></div></section>`;
