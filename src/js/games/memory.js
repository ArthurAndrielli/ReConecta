...
function render(container, callbacks) {
  ...
  container.innerHTML = `<section class="activity-card"><h2>Jogo da Memória</h2><p>Encontre os dois pares.</p><div class="memory-grid" id="memory-grid"></div><p class="feedback" id="feedback" aria-live="polite"></p><div class="actions"><button class="secondary" id="back-home">← Voltar ao início</button><button id="restart">Jogar novamente</button></div></section>`;
  ...
  const select = (index) => { if (locked || matched.includes(index) || openCards.includes(index)) return; openCards.push(index); draw(); if (openCards.length < 2) return; callbacks.onAttempt(); const [first, second] = openCards; if (deck[first] === deck[second]) { matched.push(first, second); openCards = []; callbacks.onCorrect(); callbacks.onMessage('correct'); draw(); if (matched.length === deck.length && !completed) { completed = true; callbacks.onComplete({ elapsedTime: Date.now() - startTime }); container.querySelector('#feedback').textContent = 'Parabéns! Você concluiu o Jogo da Memória.'; } } else { locked = true; callbacks.onWrong(); callbacks.onMessage('wrong'); setTimeout(() => { openCards = []; locked = false; draw(); }, 700); } };
  container.querySelector('#back-home').addEventListener('click', callbacks.onBack);
  container.querySelector('#restart').addEventListener('click', () => render(container, callbacks));
  draw();
}
export { render };
