...
function render(container, callbacks, context = {}) {
  ...
  container.innerHTML = `<section class="activity-card" data-level="${context.level || 1}"><h2>Encontre o Objeto</h2><p>${round.instruction}.</p><div class="choice-grid" id="find-options" aria-label="Objetos disponíveis">${round.options.map((item) => `<button class="choice-button" data-answer="${item}" aria-label="Objeto ${item}">${item}</button>`).join('')}</div><p class="feedback" id="feedback" aria-live="polite"></p><div class="actions"><button class="secondary" id="back-home">← Voltar ao início</button><button id="restart">Jogar novamente</button></div></section>`;
