...
function render(container, callbacks, context = {}) {
  ...
  container.querySelector('#observe-done').addEventListener('click', showOptions);
  container.querySelector('#back-home').addEventListener('click', () => { window.clearTimeout(timer); callbacks.onBack(); });
  ...
  if (preferences.observationMode === 'suggested-time') timer = window.setTimeout(showOptions, exposureTime);
}
