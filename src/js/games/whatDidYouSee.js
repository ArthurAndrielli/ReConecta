...
export function render(container, callbacks, { content, level, preferences }) {
  ...
  container.innerHTML = `<div class="observation-area"><p class="observation-note">${preferences.observationMode === 'suggested-time' ? `Observe por ${remaining / 1000} segundos. Você também pode continuar antes.` : 'Observe com calma. Quando estiver pronto, toque em “Já observei”.'}</p><div class="choice-grid">${content.items.map(id => `<div class="choice-item">${picture(id)}</div>`).join('')}</div><button id="observe-done">Já observei</button></div>`;
  callbacks.setCommand?.('Observe e memorize os objetos.');
  ...
  const show = () => {
    ...
    callbacks.setCommand?.(content.prompt);
