...
function phaseCard(phase, state, record, game) {
  ...
  return `<li class="phase-card phase-${state}"><button class="phase-card-action" data-phase="${phase.id}" ${state === 'locked' ? 'disabled' : ''} ${current ? 'aria-current="step"' : ''} aria-label="${action ? `${action} fase ${phase.ordinal} de 3: ` : ''}${escapeHTML(phase.title)}. ${status}. ${description}"><span class="phase-card-top"><span class="phase-card-icon" aria-hidden="true">${renderIcon(game.id)}</span><span class="phase-status"><span aria-hidden="true">${statusIcon}</span> ${status}</span></span><strong>Fase ${phase.ordinal} de 3</strong><h3>${escapeHTML(phase.title)}</h3><span class="phase-description">${description}</span>${action ? `<span class="phase-action-label" aria-hidden="true">${action}<span>→</span></span>` : ''}</button></li>`;
}
