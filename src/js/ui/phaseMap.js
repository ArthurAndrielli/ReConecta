...
function phaseCard(phase, state, record, game) {
  ...
  const description = state === 'completed'
    ...
    : current ? 'Pronta para continuar' : `Conclua a fase ${phase.ordinal - 1} para continuar`;
  const action = state === 'completed' ? 'Repetir' : current ? 'Começar' : '';

  return `<li class="phase-card phase-${state}"><button class="phase-card-action" data-phase="${phase.id}" ${state === 'locked' ? 'disabled' : ''} ${current ? 'aria-current="step"' : ''} aria-label="${action ? `${action} fase ${phase.ordinal} de 3: ` : ''}${escapeHTML(phase.title)}. ${status}. ${description}"><span class="phase-card-top"><span class="phase-card-icon" aria-hidden="true">${renderIcon(game.id)}</span><span class="phase-status"><span aria-hidden="true">${statusIcon}</span> ${status}</span></span><strong>Fase ${phase.ordinal} de 3</strong><span class="phase-title">${escapeHTML(phase.title)}</span><span class="phase-description">${description}</span>${action ? `<span class="phase-action-label" aria-hidden="true">${action}<span>→</span></span>` : ''}</button></li>`;
}
...
export function renderPhaseMap(container, game, progress, actions) {
  ...
  container.innerHTML = `<section class="page-card phase-map" data-game-theme="${game.id}"><button class="secondary" id="back-catalog">← Voltar às atividades</button><span class="eyebrow">Percurso de ${categories.find(c => c.id === game.category)?.name}</span><h1>${game.name}</h1><p>${game.description}</p><div class="phase-summary" aria-live="polite"><span><strong>Seu progresso</strong><span>${summary.completed} de ${summary.total} fases concluídas</span></span><progress value="${summary.completed}" max="${summary.total}" aria-label="Progresso em ${game.name}: ${summary.completed} de ${summary.total} fases"></progress></div><div class="actions"><button id="continue-phase" ${next ? '' : 'disabled'}>${next ? `Continuar na fase ${next.ordinal}` : 'Escolha uma fase para repetir'}</button><button class="secondary" id="free-practice">Prática livre</button></div><div class="phase-blocks">${[1, 2, 3].map((block) => `<section aria-labelledby="phase-block-${block}"><h2 id="phase-block-${block}">${['Fácil', 'Médio', 'Difícil'][block - 1]}</h2><ol class="phase-grid">${phases.filter((phase) => phase.block === block).map((phase) => { const state = getPhaseState(progress, game.id, phase.ordinal); const record = progress.phaseProgress?.[game.id]?.[phase.ordinal]; return phaseCard(phase, state, record, game); }).join('')}</ol></section>`).join('')}</div></section>`;
